...
class BaseWebElement:
	...
	def write(self, *content) -> None:
		...
		示例:
			write("test") \n
			write(Keys.CONTROL,"a")
		"""
		formated_content = str(content)[1:-1].rstrip(",").replace(",", "+").replace(" ", "")

		try:
			self.scroll_into_view()
			self.element.send_keys(*content)

			msg = f"向{self.name}元素中输入内容{formated_content}成功"
			allure.attach(msg, name="输入", attachment_type=allure.attachment_type.TEXT)
			logger.debug(msg)
		except Exception as why:
			msg = f"向元素{self.name}中输入内容{formated_content}失败，原因:{why}"
			allure.attach(msg, name="输入", attachment_type=allure.attachment_type.TEXT)
			logger.error(msg)
			self.save_screenshot()
			raise Exception(msg)

	def click(self, waited_config: dict = None) -> None:
		"""
		点击元素
	...
	def switch_to_frame(self, waited_config: dict = None) -> None:
		"""
		切换到frame
		...
		try:
			...
			msg = f"切换{self.name}frame元素失败，原因:{why}"
			allure.attach(msg, name="frame", attachment_type=allure.attachment_type.TEXT)
			logger.error(msg)
			self.save_screenshot()
			raise Exception(msg)
	...
	def scroll_into_view(self) -> None:
		...
		try:
			...
			logger.debug(f"滚动到{self.name}元素可见位置成功")
		except Exception as why:
			logger.error(f"滚动到{self.name}元素可见位置失败，原因:{why}")
			self.save_screenshot()
			raise Exception(why)
	...
	def select(self, value, method="index"):
		...
		try:
			...
			if method == "index":
				...
				logger.debug("select下拉框/复选框元素根据index选择成功")
			elif method == "value":
				select.select_by_value(value)
				logger.debug("select下拉框/复选框元素根据value选择成功")
			elif method == "text":
				select.select_by_visible_text(value)
				logger.debug("select下拉框/复选框元素根据文本选择成功")
			else:
	...
	def deselect_all(self) -> None:
		...
		try:
			...
			logger.debug("select下拉框/复选框全部元素取消选择成功")
		except Exception as why:
			logger.debug(f"select下拉框/复选框全部元素取消选择失败，原因：{why}")
			self.save_screenshot()
			raise Exception(why)
	...
	def save_screenshot(self) -> None:
		...
		:return: None
		"""
		if self.screenshot is False:
			logger.debug("元素截图未开启")
			return
	...
	def mouse_hover(self) -> None:
		...
		try:
			...
			logger.debug("在元素上悬停成功")
		except Exception as why:
			logger.debug(f"在元素上悬停失败，原因：{why}")
			self.save_screenshot()
			raise Exception(why)

	def drag_and_drop_by_offset(self, xoffset, yoffset) -> None:
		"""
		将元素拖动到指定位置
		...
		try:
			...
			logger.debug("将元素拖拽到指定位置成功")
		except Exception as why:
			logger.error(f"将元素拖拽到指定位置失败，原因：{why}")
			self.save_screenshot()
			raise Exception(why)

	def drag_and_drop(self, target_element) -> None:
		"""
		将元素拖动到另一个元素上
...
class BaseWebDriver:
	...
	def get(self, url: str, waited_method: str = None, waited_config: dict = None) -> None:
		"""
		访问url地址
	...
	def find_element(self, by=By.XPATH, value=None, name: str = "", waited_config: dict = None) -> BaseWebElement:
		"""
		查找单个元素
	...
	def switch_to_alert(self, scanner: str = None, name: str = "", waited_config: dict = None) -> None:
		"""
